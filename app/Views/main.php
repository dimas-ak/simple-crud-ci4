...
<head>
	...
	<link rel="shortcut icon" type="image/png" href="<?= site_url("public") ?>/favicon.ico"/>
    ...
    <link rel="stylesheet" href="<?= site_url("public/css/arjunane.min.css")?>">
</head>
<body>
    <?PHP if($login): ?>
        <header>
    ...
    <script>
        var form_error  = document.getElementsByClassName("form-error"),
            delete_udud = document.getElementsByClassName("delete");

        for(var d = 0; d < delete_udud.length; d++)
        {
            delete_udud[d].addEventListener("click", function (x) {
                x.preventDefault();
                var tr          = this.parentElement.parentElement,
                    url         = this.getAttribute("href"),
                    nama_udud   = tr.getElementsByTagName("td")[1].innerHTML;
                    
                if(confirm('Apakah Anda yakin ingin menghapus data : "' + nama_udud + '" ?'))
                {
                    ...
                    xhttp.onreadystatechange = function() 
                    {
                        if (this.readyState == 4 && this.status == 200) 
                        {
                            ...
                            if(json.result)
                            {
                                ...
                                var update_tr = document.getElementsByTagName("tbody")[0].getElementsByTagName("tr");
                                console.log(update_tr)
                                for(var ut = 0; ut < update_tr.length; ut++)
                                {
                                    update_tr[ut].getElementsByTagName("td")[0].innerHTML = ut + 1;
                                }
                            }
                            else
                            {
        ...
        for(var fe = 0; fe < form_error.length; fe++)
        {
            form_error[fe].addEventListener("click", function () {
                this.remove();
            });
        }
        function toggleMenu() {
